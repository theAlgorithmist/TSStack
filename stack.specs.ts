...
import {TSMT$Stack} from './src/Stack';
import {STACK_TYPE} from './src/Stack';

// Test Suites
describe('TSMT Stack', () => {
  let __numStack: TSMT$Stack<number> = new TSMT$Stack<number>();
  ...
  it('correctly constructs, modifies a stack, and then returns a copy as an array', function() {
    ...
    let arr: Array<number> = __numStack.toArray();
    expect(arr[0]).toBe(2);
    expect(arr[1]).toBe(3);
